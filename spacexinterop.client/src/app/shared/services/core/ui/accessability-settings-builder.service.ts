  ...
  private async startBuilding(): Promise<IAccessibilitySettingItem[]> {
    return new Promise(async (resolve, reject) => {
      try 
      {
        ...
        const highContrast: IAccessibilitySettingItem = { selfIdentifier: `high_contrast`, title: 'High Contrast', ariaLabel: 'high_contrast', matIcons: MatIcons.Contrast, changes: this.filterSettingClicked.bind(this), filterStyle: "contrast(130%)" };
        const monochrome: IAccessibilitySettingItem = { selfIdentifier: `monochrome`, title: 'Monochrome', ariaLabel: 'monochrome', matIcons: MatIcons.Monochrome, changes: this.filterSettingClicked.bind(this), filterStyle: "grayscale(100%)" };
        const invertColors: IAccessibilitySettingItem = { selfIdentifier: `invert_colors`, title: 'Invert Colors', ariaLabel: 'invert_colors', matIcons: MatIcons.InvertColors, changes: this.filterSettingClicked.bind(this), filterStyle: "invert(100%)" };
        const largeText: IAccessibilitySettingItem = { selfIdentifier: `large_text`, title: 'Large Text', ariaLabel: 'large_text', matIcons: MatIcons.LargeText, changes: this.largeTextClicked.bind(this) };
  ...
  private settingClicked(event: SelectionChange, setting: IAccessibilitySettingItem): Promise<boolean> {
    return new Promise(async (resolve, reject) => {
      try 
      {
        ...
        if (event instanceof MouseEvent) { state = !state; }
      ...
      try 
      {
        ...
        if(setting.filterStyle) {
          if(state) {
            ...
            this.htmlElement.style.filter = this.htmlElement.style.filter.replace(setting.filterStyle, EMPTY_STRING);
          }
        }
